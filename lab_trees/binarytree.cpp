...
bool BinaryTree<T>::isOrderedIterative() const
{
   ...
   typename BinaryTree<T>::Node* temp = NULL;
...
void BinaryTree<T>::getPaths(vector<vector<T> > &paths) const
{
   ...
   if(root!=NULL){
     vector<T> one;
     getPathH(root,paths,one);
   }
   return ;
}

template <typename T>
void BinaryTree<T>::getPathH(Node* subRoot,vector<vector<T>> &paths,vector<T> one) const{
 typename BinaryTree<T>::Node* temp = subRoot;
  if(temp ==NULL){
    return;
  }
      one.push_back(temp->elem);
  ...
  getPathH(subRoot->left,paths,one);
  getPathH(subRoot->right,paths,one);
  return;
}
...
int BinaryTree<T>::sumDistances(Node* subRoot,int count) const{
  if (subRoot == NULL){
      ...
      int total = sumDistances(subRoot->left, count+1) + sumDistances(subRoot->right,count+1)+count;
