...
class KDTree
{
  ...
  private:
     ...
     */
     ...
     KDTreeNode * build(vector<Point<Dim>>& points,int begin,int end,int d);
     void median(vector<Point<Dim>>& points,int begin,int end,int mid,int d);
     int partition(vector<Point<Dim>>& points, int begin, int end,int cen, int d);
     void swap(vector<Point<Dim>>& points,int one, int two);
     void Delete(KDTreeNode* r);
     vector<Point<Dim>> points;

     double getDistance(const Point<Dim> &first, const Point<Dim>& second) const;

     Point<Dim> findNearestHelper(int curDim,int begin, int end,const Point<Dim>& currentBest,const Point<Dim>& query)const;
};
