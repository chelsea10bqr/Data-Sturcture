     ...
     */
     ...
     if(shouldResize()){
         ...
         while(table[index]!= NULL){
           count++;
           index = (index+count*secondary_hash(key,size))%size;
         }
...
int DHHashTable<K, V>::findIndex(const K& key) const
{
     ...
     */
     int count = 0;
     size_t idx =(hash(key,size)+count*secondary_hash(key,size))%size;
    size_t start = idx;
    while(should_probe[idx]){
      ...
      count++;
      idx = (hash(key,size)+count*secondary_hash(key,size))%size;
    }
    return -1;
}
