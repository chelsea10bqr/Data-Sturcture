...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    string t1 = word1;
    string t2 = word2;
    std::transform(word1.begin(),word1.end(),t1.begin(),toupper);
    std::transform(word2.begin(),word2.end(),t2.begin(),toupper);
    if(dict.find(t1)!=dict.end()&&dict.find(t2)!=dict.end()){
      if(dict.find(t1)->second==dict.find(t2)->second){
        return true;
      }
    }else{
      return false;
